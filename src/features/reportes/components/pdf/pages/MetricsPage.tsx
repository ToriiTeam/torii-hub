...
export function MetricsPage({ metrics, trend }: MetricsPageProps) {
  ...
  return (
    <PdfPage pageNumber={2}>
      ...
      <div style={{ width: 40, height: 3, background: '#e5182b', marginBottom: 24 }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 32 }}>
        {cards.map((c) => (
          ...
          >
            <p style={{ fontSize: 10, color: '#888', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              {c.label}
            </p>
            <p style={{ fontSize: 20, fontWeight: 700, margin: 0, color: '#1a1a1a' }}>{c.value}</p>
          </div>
        ))}
      </div>

      <h3 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 12px', color: '#1a1a1a' }}>
        Tendencia de inversión y leads (por semana)
      </h3>
      {chartData.length > 0 ? (
